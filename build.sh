...
mvn install
cd target 
unzip ets-sfs12-1.2.1-r4.zip -d $TE_BASE/scripts/
...
echo "  "
echo "TEAM Engine ($te_tag) and the ets-sfs12-1.2.1-r4 have been installed."
echo "  "
echo " UNIX/OS users - You can run the test with the following commands: "
echo "   export TE_BASE=$TE_BASE"
echo "   $base/teamengine/te_install/bin/unix/test.sh -source=sfs/1.2.1/ctl/sfs-suite.xml"
echo "  "
echo " Windows users - You can run the test with the following commands:"
echo "   set TE_BASE=$TE_BASE"
echo "   $base/teamengine/te_install/bin/windows/test.bat -source=sfs/1.2.1/ctl/sfs-suite.xml"
echo "  "
